#!/bin/bash
# Create animated ConveniencePro Instagram ad - Mobile Portrait 9:16
# White background, sleek font, 50% transparent C behind text

cd "$(dirname "$0")"

# Create blank white slide for initial fade-in (portrait 1080x1920)
ffmpeg -f lavfi -i color=c=white:s=1080x1920:d=1 -frames:v 1 -y blank.png
...
echo "Creating Slide 3 animation (ConveniencePro fades in, then C behind at 50% opacity)..."
# Slide 3: ConveniencePro fades in first, then C fades in behind at 50% transparency
# - 0.0-1.0s: Fade in ConveniencePro text
# - 1.0-2.0s: Fade in C behind (at 50% opacity)
# - 2.0-3.0s: Hold
...
echo ""
echo "Mobile portrait video (9:16) successfully created!"
echo "Dimensions: 1080x1920"
echo "Features: Sleek font, 50% transparent C layered behind text"
