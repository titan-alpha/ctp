...
export function useBmpToPng(): UseBmpToPngReturn {
  ...
  const convert = useCallback(async (files: File[]) => {
    ...
    const bmpFiles = files.filter(
      (f) => f.type === 'image/bmp' || f.name.toLowerCase().endsWith('.bmp')
    );

    if (bmpFiles.length === 0) {
      setError('Please select BMP files');
      setIsConverting(false);
      return;
    }

    try {
      const results = await Promise.all(bmpFiles.map(convertSingleFile));
