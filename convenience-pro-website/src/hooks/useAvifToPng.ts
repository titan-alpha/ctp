...
export function useAvifToPng(): UseAvifToPngReturn {
  ...
  const convertSingleFile = useCallback(async (file: File): Promise<ConvertedImage> => {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        // Clear canvas and draw image (preserves transparency)
  ...
  const convert = useCallback(async (files: File[]) => {
    ...
    const avifFiles = files.filter(
      (f) => f.type === 'image/avif' || f.name.toLowerCase().endsWith('.avif')
    );

    if (avifFiles.length === 0) {
      setError('Please select AVIF files');
      setIsConverting(false);
      return;
    }

    try {
      const results = await Promise.all(avifFiles.map(convertSingleFile));
